...
BriefOnce can register its local MCP server for Codex CLI, the Codex IDE extension, and the ChatGPT desktop app, then install a repository-root lifecycle region for Codex.
...
BriefOnce parses TOML but edits only between:
...
Exact config backups live in restrictive BriefOnce user state outside the repository. Backup contents are never printed or placed in project files.

## `AGENTS.md` lifecycle

BriefOnce owns only this region in repository-root `AGENTS.md`:
...
User content outside the region is preserved byte-for-byte where practical. Missing files are created; existing files are appended; identical regions are unchanged; modified or duplicate regions are conflicts. BriefOnce does not install a Codex skill, plugin, hook, `notify` handler, or IDE extension.

## Worktrees

BriefOnce distinguishes the main checkout from linked worktrees using read-only `git rev-parse` commands. Every worktree is a separate mutable workspace and gets independent `AGENTS.md` ownership. Only a hash of the common Git directory is retained as a repository-family identifier.
...
- complete BriefOnce initialization;
...
- all nine BriefOnce tools and `agentfold_get_status`;
...
Use `b1 reliability --host codex` to inspect only Codex sessions that
actually contacted BriefOnce. A missing session cannot be treated as proof that
